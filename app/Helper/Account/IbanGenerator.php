...
class IbanGenerator
{
    ...
    public function generate($num_pays, $user)
    {
        ...
        $bban = "".$code_banque.$code_branche.$numero_cpt.$key.base_convert(substr($pays, 0, 1), 36, 10).base_convert(substr($pays, 1, 1), 36, 10)."00";
        $cleIban = '98' - bcmod($bban, 97);

        $rib = "".$code_banque.$code_branche.$numero_cpt.$key."";
        ...
        try {
            return Iban::create([
                ...
                "code_guichet" => $code_branche,
                "number_account" => $numero_cpt,
                "key_rib" => $key,
                "user_id" => $user->id
            ]);
